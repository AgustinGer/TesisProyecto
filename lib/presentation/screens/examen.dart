...
/*
class QuizScreen extends ConsumerStatefulWidget {
  final int quizInstanceId; // ID de la instancia (module['instance'])
  final int courseId;
  final int cmid; // ID del módulo (para el link web)
  final String title;
...
class _QuizScreenState extends ConsumerState<QuizScreen> {
  ...
  Future<void> _cargarInfoExamen() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Moodle devuelve una lista de TODOS los exámenes del curso.
        // Tenemos que buscar el que coincida con nuestro ID.
        if (data is Map && data.containsKey('quizzes')) {
          final List quizzes = data['quizzes'];
          final myQuiz = quizzes.firstWhere(
            (q) => q['id'] == widget.quizInstanceId,
          ...
          if (myQuiz != null) {
            ...
            setState(() {
              _errorMessage = "No se encontró información del examen.";
              _isLoading = false;
            });
          }
        } else {
           // Si falla la API (como con el Libro), mostramos la info básica
           setState(() {
             _errorMessage = "No se pudo cargar detalles, pero puedes abrirlo.";
             _isLoading = false;
           });
        }
      }
  ...
  Future<void> _abrirExamenWeb() async {
    ...
    // URL directa al examen
    final url = '$baseUrl/mod/quiz/view.php?id=${widget.cmid}';
    
    await launchUrl(
      Uri.parse(url),
      mode: LaunchMode.externalApplication, // Abrir en Chrome/Navegador externo
    );
  }
  ...
  // Helper para formatear tiempo límite
  String _formatDuration(int? seconds) {
    if (seconds == null || seconds == 0) return "Sin límite";
    final duration = Duration(seconds: seconds);
    return "${duration.inMinutes} minutos";
          ...
          : Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // SI HAY DATOS DEL API, LOS MOSTRAMOS
                  if (_quizData != null) ...[
                    // Intro / Descripción
                    if (_quizData!['intro'] != null)
                      Container(
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(
                        ...
                        child: Html(data: _quizData!['intro']),
                      ),
                    ...
                    Row(
                      children: [
                        Expanded(child: _InfoCard(
                          icon: Icons.timer, 
                          label: "Límite", 
                          value: _formatDuration(_quizData!['timelimit'])
                        )),
                        const SizedBox(width: 10),
                        Expanded(child: _InfoCard(
                          icon: Icons.repeat, 
                          label: "Intentos", 
                          value: _quizData!['attempts'] == 0 ? "Ilimitados" : "${_quizData!['attempts']}"
                        )),
                      ],
                    ),
                    const SizedBox(height: 10),
                    _InfoCard(
                      ...
                      label: "Cierra", 
                      value: _formatDate(_quizData!['timeclose'])
                    ),
                  ] 
                  // SI HUBO ERROR O API FALTANTE (Fallback)
                  else if (_errorMessage != null) ...[
                    Text(
                      _errorMessage!,
                      textAlign: TextAlign.center,
                  ...
                  // BOTÓN DE ACCIÓN
                  const Text(
                    "Para garantizar la estabilidad de tu conexión y el guardado de respuestas, el examen se abrirá en el navegador.",
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 12, color: Colors.grey),
                  ),
                  const SizedBox(height: 10),
...
// Widget auxiliar para las tarjetitas
class _InfoCard extends StatelessWidget {
  final IconData icon;
  final String label;
  final String value;
